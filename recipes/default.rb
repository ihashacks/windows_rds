...
if node[:windows_rds][:certcn]
	powershell "Set RDS certificate" do
	  code <<-EOH
	  $modules = Get-Module -ListAvailable | Select-Object Name
	  $DesiredCert = (Get-ChildItem cert:/localmachine/my | Where-Object {$_.Subject -match "^CN=#{node[:windows_rds][:certcn]}"}).Thumbprint
...
if node[:windows_rds][:remoteapp_hostname]
	powershell "Set RemoteApp Hostname" do
		...
		not_if { reg_value = Registry.get_value('HKLM\Software\Microsoft\Windows NT\CurrentVersion\Terminal Server\TSAppAllowList','DeploymentRDPSettings')
			      !reg_value.nil? and reg_value.match("full address:s:#{node[:windows_rds][:remoteapp_hostname]}") }
		code <<-EOH
		Import-Module RemoteDesktopServices
		Set-Item RDS:/RemoteApp/ServerName #{node[:windows_rds][:remoteapp_hostname]}
		EOH
	end
end